...
This package comes with some bundled reference data that can be used for demonstrational purposes and in off-line situations. For real use cases, it is recommended to use on-line services or tools for getting up-to-date reference data, such as the rgbif R package.
...
# How to apply the above cleanup function across columns, dplyr style
hcaf_eu <- 
  hcaf %>%
...
jagged_tooth <- "galemys pyrenaicus"
...
ggplot(world, aes(x = long, y = lat, group = group)) +
  ...
  # add point data
  geom_point(data = gws, aes(x = gws$decimalLongitude, y = gws$decimalLatitude, group = 0),
...
hdc_world <- 
  tbl_df(get(data("aquamapsdata_Hdc_grid"))) %>%
...
fisher_intervals <- classInt::classIntervals(df$p, n = 5, style = "fisher")
