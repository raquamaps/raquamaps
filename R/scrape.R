...
nativerange <- function(latinname = default_species(), identifier) {
  ...
  if (length(ids) > 1) {
    warning(
      "More than one match found for ", latinname,
  ...
  if (length(maps) < 1) {
    stop(
      "Found no native range data for ", latinname,
      " at ", res$url
    )
  }
...
parse_am_csv <- function(url) {
  ...
  extract_df <- function(beg, end) {
    res <- utils::read.csv(
      file = textConnection(rows[beg:end]),
...
get_am_name_uris <- function(latinname = default_species()) {
  ...
  resolver_uri <-
    paste0(
      "http://aquamaps.org/ScientificNameSearchList.php?",
